...
function enter(pi) {
    ...
    var em = pi.getEventManager("FairybossBattle"); //BOSS事件名称

    if (pi.getPlayerCount(300030310) <= 0) {//BOSS地图无人
        ...
        if (party == null) {
            ...
            if (party.getLeaderId() != player.getId()) {
                ...
                for (var i = 0; i < members.size(); i++) {
                    ...
                    if (chr.getQuestStatus(31228) != 2) {
                        canGoIn = false;
                        cause = chr.getName() + "没完成前置任务,无法进入";
                        break;
                    }
					//if (chr.getBossLog(0, "挑战艾菲尼娅") >= timeLimit) {  //次数限制暂无法使用
					//	canGoIn = false;
                ...
                if (canGoIn) {
					...
					//	members.get(i).setBossLog(0, "挑战艾菲尼娅");
					//}
                        return true;
                    } else {
