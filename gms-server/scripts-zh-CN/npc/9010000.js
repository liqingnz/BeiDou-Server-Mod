...
function start() {
    cm.sendSimple("嗨，我是#p9010000#，有什么可以帮忙的？？ \r\n\r\n#b#L0#兑换35级枫叶武器。#l \r\n\r\n#L1#兑换43级枫叶武器。#l\r\n\r\n#L2#兑换64级枫叶武器。#l#k");
	return;
}
    ...
    if (status == 0) {
        ...
        if (sel == 2 || sel == 4) {
            if (mode != 1) {
                ...
                if ((cm.getMeso() < cost) || (!cm.haveItem(oldWepId, 1)) || (!cm.haveItem(4030012, leaves))) {
                    cm.sendOk("很抱歉，由于您的材料不足所以无法帮您制作，假如需要的话可以再来找我谈谈。");
                    cm.dispose();
                } else if (stimulator == null || !cm.haveItem(stimulator)) {
                    if (cm.canHold(newWepId)) {
                        cm.gainItem(oldWepId, -1);
                        cm.gainItem(4030012, -leaves);
                        cm.gainMeso(-cost);
            ...
            if ((cm.getMeso() < cost) || !cm.haveItem(4030012, leaves)) {
                cm.sendOk("很抱歉，由于您的材料不足所以无法帮您制作，假如需要的话可以再来找我谈谈。");			
            } else {
                if (cm.canHold(newWepId)) {
                    cm.gainItem(4030012, -leaves);
            ...
            if ((cm.getMeso() < cost) || !cm.haveItem(4030012, leaves) || !cm.haveItem(oldWepId, 1)) {
                cm.sendOk("很抱歉，由于您的材料不足所以无法帮您制作，假如需要的话可以再来找我谈谈。");
            } else {
                if (cm.canHold(newWepId)) {
					cm.gainItem(oldWepId, -1);
