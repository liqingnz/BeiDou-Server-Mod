...
function action(mode, type, selection) {
    if (mode < 1)  // disposing issue with stylishs found thanks to Vcoc
    {
        ...
        if (status == 0) {
            cm.sendSimple("嗨，你好！欢迎来到射手村整形外科！你想把你的脸变成全新的样子吗？使用 #b#t5159002##k，你可以让我们来照顾剩下的事情，拥有你一直想要的脸~！\r\n#L2#整形外科：#i5159002##t5159002##l");
        } else if (status == 1) {
            if (selection == 2) {
                facenew = Array();
                ...
                cm.sendStyle("让我看看... 我完全可以把你的脸变成新的。 你不想试试吗？ 使用 #b#t5159002##k, 花点时间选择你可以得到你喜欢的面孔。", facenew);
            }
        } else if (status == 2) {
            cm.dispose();
