...
function process(p) {
	if (p == null || p.getMap() == null)
		return;
	var mos = p.getMap().getItems();
	for (var i = 0; i < mos.length; i++) {
		if (mos[i].isPlayerDrop())
			continue;
		if (mos[i].getMeso() <= 0) {
			if (exclude_equip(mos[i], p)) 
				continue;
			if (!exclude_check(mos[i], p)) {
				mos[i].sendDestroyData(p.getClient());
				continue;
			}
			if (check_full(mos[i], p))
				continue;
		}			
...
function exclude_check(mo, p) { // 返回false, 则不捡, 且消除物品
    if (exIds.has(mo.getItemId()))
		return false;
	if (mo.isPickedUp()) 
		return false;
	if (mo.getQuest() > 0 && !p.needQuestItem(mo.getQuest(), mo.getItemId())) {
		return false;
...
function check_full(mo, p) { // 检测背包是否满了
	var itemId = mo.getItemId();
	if (itemId == 4031865 || itemId == 4031866)
		return false;
	var t = ItemConstants.getInventoryType(itemId).name();
	if (p.getInventory(InventoryType.ETC).isFull()) {
		if (t == "ETC" && !p.haveItem(itemId))
			return true; // 不可捡
	}
	return false;
}
