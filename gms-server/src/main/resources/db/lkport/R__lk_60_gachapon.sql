-- ----------------------------------------------------------------------------
-- LichKingMod -> BeiDou 移植：扭蛋奖池（gachapon_reward / _pool）
-- ---------------------------------------------------------------------------
--
...
--
-- 【幂等性】奖池按 (gachapon_id, name) 先查后插；奖品按 (pool_id, item_id) 先删后插。
-- 【顺序】神木村小节要从再平衡之后的 1-36 号池里取公共奖品，两节必须留在同一文件且保持先后。
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------- 普通档
-- 先查后插：奖池已存在就复用它的 id，不再插一个同名池（本文件可重放）。
SET @pool_id = (SELECT `id` FROM `gachapon_reward_pool` WHERE `gachapon_id` = 9100111 AND `name` = '神木村(普通)' LIMIT 1);
INSERT INTO `gachapon_reward_pool` (`name`, `gachapon_id`, `weight`, `is_public`, `prob`, `start_time`, `end_time`, `notification`, `comment`)
SELECT '神木村(普通)', 9100111, 9000, 0, 0, NOW(), NULL, 0, 'LK' FROM DUAL WHERE @pool_id IS NULL;
SET @pool_id = COALESCE(@pool_id, LAST_INSERT_ID());
-- 奖品整池重建，避免重跑时把同一道具越堆越多（等同于加权）。
DELETE FROM `gachapon_reward` WHERE `pool_id` = @pool_id;
...
-- ---------------------------------------------------------------- 稀有档
-- 先查后插：奖池已存在就复用它的 id，不再插一个同名池（本文件可重放）。
SET @pool_id = (SELECT `id` FROM `gachapon_reward_pool` WHERE `gachapon_id` = 9100111 AND `name` = '神木村(稀有)' LIMIT 1);
INSERT INTO `gachapon_reward_pool` (`name`, `gachapon_id`, `weight`, `is_public`, `prob`, `start_time`, `end_time`, `notification`, `comment`)
SELECT '神木村(稀有)', 9100111, 800, 0, 0, NOW(), NULL, 0, 'LK' FROM DUAL WHERE @pool_id IS NULL;
SET @pool_id = COALESCE(@pool_id, LAST_INSERT_ID());
-- 奖品整池重建，避免重跑时把同一道具越堆越多（等同于加权）。
DELETE FROM `gachapon_reward` WHERE `pool_id` = @pool_id;
INSERT INTO `gachapon_reward` (`pool_id`, `item_id`, `quantity`, `create_time`, `comment`) VALUES
...
-- ---------------------------------------------------------------- 传奇档
-- 先查后插：奖池已存在就复用它的 id，不再插一个同名池（本文件可重放）。
SET @pool_id = (SELECT `id` FROM `gachapon_reward_pool` WHERE `gachapon_id` = 9100111 AND `name` = '神木村(传奇)' LIMIT 1);
INSERT INTO `gachapon_reward_pool` (`name`, `gachapon_id`, `weight`, `is_public`, `prob`, `start_time`, `end_time`, `notification`, `comment`)
SELECT '神木村(传奇)', 9100111, 200, 0, 0, NOW(), NULL, 1, 'LK' FROM DUAL WHERE @pool_id IS NULL;
SET @pool_id = COALESCE(@pool_id, LAST_INSERT_ID());
-- 奖品整池重建，避免重跑时把同一道具越堆越多（等同于加权）。
DELETE FROM `gachapon_reward` WHERE `pool_id` = @pool_id;
INSERT INTO `gachapon_reward` (`pool_id`, `item_id`, `quantity`, `create_time`, `comment`) VALUES
