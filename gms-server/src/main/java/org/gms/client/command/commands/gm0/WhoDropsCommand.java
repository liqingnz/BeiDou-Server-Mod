 ...
 * 一屏根本装不下。<b>装不下的部分不是被服务端截掉的</b>——{@code npcTalk} 一路到
 * {@code ByteBufOutPacket.writeString} 全程没有任何长度检查，协议这边的硬上限是
 * 长度域那 2 个字节能表达的 65535 字节（还得注意中文按客户端字符集算一个字 2 字节）；
 * 真正的边界是客户端对话框高度固定又没有滚动条，画不下的行压根不画。
 * <p>
