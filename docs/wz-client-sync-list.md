...
| 项 | 说明 |
|---|---|
| `Item.wz/Etc/0403` 的 `04033001` | **已补**（用户从 LK 客户端提取，服务端随之合入）。注：它在三方 `String.wz/Etc.img` 都没有名字，LK 的 `handbook`、`Quest.wz`、脚本与 sql 也零引用——**LK 用作 BOSS 掉落的 5000 点券其实是 `4310100`**，见下 |
| **`Item.wz/Etc/0431` 的 `04310100`** | **缺口**：`V1000.0.1` 迁移已写入它的 `drop_data`（拉皮条 45000、马来 BOSS 各 80000、扎昆 300000、暗黑龙王 500000），但三方 `Item.wz/Etc/0431.img` 与 `String.wz/Etc.img` 都没有这个道具——掉出来会是 `getSlotMax` 返回 0 的废物。待处理 |
