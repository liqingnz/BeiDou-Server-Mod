...
### 24.11 `@whodrops` 改成翻页（2026-08-20）

掉落源多的物品实测能到 **837 个**（`drop_data` 里 180 件物品的掉落源超过 30 个），
原先的做法是「只列前 N 条 + 末尾说一句还剩多少」——剩下的永远看不到。改成真翻页。

**为什么必须走脚本**：翻页要求「点了之后还能回到服务端」，而 `npcTalk` 是一发了之的对话框，
没有回调。所以整条链路挂到 `scripts[-zh-CN]/npc/whoDropsList.js` 上，
指令只负责查数据、开会话、拉起脚本。

**分工**：数据、掉率折算、立绘、每页条数、i18n 全在 `WhoDropsCommand`；
脚本只摆一页正文、加翻页控件、把点击回传。会话（候选物品 + 掉落源）挂在角色 id 上，
翻页不会把 `getItemDataByName` 那趟全表模糊匹配或 `drop_data` 查询重跑一遍。
对话框关掉时 `endSession` 清掉，别把整份掉落源挂在内存里。

供脚本调用的接口：`getChoices` / `selectItem` / `getPageCount` / `renderPage` / `endSession`。

**一页放多少（`DROPPERS_PER_PAGE = 12`）仍然是估的**——服务端量不到客户端的渲染高度，
只能按「一条一行、行首一张比文字行高的立绘」推。这个数改动不影响翻页本身，嫌少嫌多直接调。

> 参照过 `@droptable`（`当前地图掉落_物品查询.js`）的做法：它的**物品列表**有翻页
> （`ITEMS_PER_PAGE = 50` + `#L9000001/9000002`），**掉落详情没有**，靠 SQL `LIMIT 30` 截断。
> 两处分段都是写死条数、不做任何测量，截掉了也不吭声。查它的时候顺带量出两个数字，
> 记在这里备查（**本轮未改那个脚本**）：
> - 详情页 `LIMIT 30`：180 件物品的掉落源超过 30，最多一件 837；且每个掉落源占 3 行，
>   30 个就是 90 行，远超一屏。
> - 列表页 SQL `LIMIT 200`：11 个分类里 5 个超标——消耗品 1130 件（只够得着 18%）、
>   其他物品 849、防具类 692、卷轴/强化 354、战士武器 225。而且 `LIMIT` 前没有 `ORDER BY`，
>   取到哪 200 条由 MySQL 决定，再在 JS 里按名字排序，所以看到的并不是「前 200 个」。

#### 24.11.1 翻页控件改用客户端自带的「上一步／下一步」

第一版是拿 `#L9000001/9000002` 拼假选项当翻页按钮。`NPCConversationManager` 其实已经有现成的：

| send 变体 | endBytes | 出现的按钮 | 对应 level 方法 |
|---|---|---|---|
| `sendNextLevel` | `00 01` | 下一步 | `nextLevel` |
| `sendLastLevel` | `01 00` | 上一步 | `lastLevel` |
| `sendLastNextLevel` | `01 01` | 两个都有 | 按 mode 分派 |
| `sendOkLevel` | `00 00` | 确定 | `nextLevel` |

`NPCScriptManager.nextLevel` 按 mode 分派：**0=上一步、1=下一步、-1=结束对话**，
回调不带参数（页码留在脚本变量里就行）。

好处不只是省几行：`#L` 假选项要编一套大数字 id 去避开物品 id，还要占正文行数
（而正文行数正是这里最紧张的资源）；原生按钮在客户端固定位置，不占正文。
另外**按钮该不该出现由每页自己挑 send 变体**——首页没有上一步、末页没有下一步——
所以不会出现「点了下一步却什么也没有」。首页的「上一步」留给「返回物品列表」，
只在从选单进来时才出现。

**顺带去掉一处内存悬挂**：会话原先放在服务端的静态表里，靠脚本 `endSession` 清。
但玩家用右上角叉掉对话框走的是 `NPCScriptManager.dispose`，那条路**不回调脚本**，
脚本没机会通知服务端，静态表就会留下一份最多几百条的掉落源。
改成指令把 `Query` 句柄交给脚本（`takeQuery` 取走即从交接台删除），
之后每次调用再传回来——句柄挂在脚本变量上，`resetContext` 一执行就没了，不用惦记回收。

**`DROPPERS_PER_PAGE` 提到 25，并给每条加了跨页连续的序号**：
页首写明「第 X-Y 条，共 N 个」，页内每条带序号，看得见的最后一个序号就是这一页的实际容量，
用来实测客户端到底能画多少行。

#### 24.11.2 物品选单也要翻页

漏了一半：掉落源翻页做了，**物品选单没做**。原先 `MENU_LIMIT = 30` 直接把候选砍到前 30 件，
后面的够不着；更糟的是脚本显示的「搜到 N 件」用的是**砍完之后**的条数，
所以搜出 500 件也只会说「搜到 30 件」——看着像本来就只有 30 件。

改法与掉落源一致：不截断，分页。`ITEMS_PER_PAGE = 25` 与 `DROPPERS_PER_PAGE` 分开——
选单一行是物品图标 + 名字，比带怪物立绘的一行矮，但要多留两行给翻页项。
标题改用真实总数（`getChoiceCount`），`WhoDropsCommand.message7`（「只列前 N 件」的告警）随之删掉。

**这里不能用原生的上一步／下一步**：选单走的是 `sendNextSelectLevel` → `sendSimple`，
endBytes 是空的，那种对话框只认 `#L` 选项，压根不画上一步／下一步按钮。
所以选单的翻页仍然是 `#L` 拼的（选项值用大正数 9000001/9000002，避开页内下标 0..n-1），
而掉落源页因为不需要 `#L` 选项，可以用原生按钮。两处形态不同是对话框类型决定的，不是没统一。

供脚本调用的接口相应变成：`getChoiceCount` / `getChoicePageCount` / `getChoices(query, page)`。
