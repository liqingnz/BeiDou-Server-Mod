...
> **移植期用分号分批（批次 6 用 `V1000.0.x`、批次 7 用 `V1000.1.x`），收尾时先合并为单文件，
> 后又按功能拆开（`2c0f42b19`）。**
> 现在 `db/lkport/` 下是 `V1000.2.1__lk_schema.sql`（建表）+ 六个 `R__lk_*.sql` 可重复迁移
> （指令 / 反作弊配置 / 玩法配置 / 掉落 / 经济 / 扭蛋）。数据层改用 `R__` 后，
> 改一行就能重跑，不必再递增版本号。第 10 节记的是合并为单文件那一步的历史。
...
**GM 不算人**（2026-08-21 追加）：`gmLevel > 1` 的角色一律排除在刷怪计算之外，
等同于不在本图——既不计入「有效玩家数」，也不会用自己的等级顶高 maxLevel 把在场的
低等级玩家挤出有效玩家范围；**图里只剩 GM 时 `respawn()` 直接当空图返回，完全不补怪**。
调参实测因此必须用非 GM 角色，挂着 GM 号站在图里是数不出刷怪率的。
入口是 `MapleMap#getSpawnRelevantPlayers` 与 `MapleMap#hasSpawnRelevantPlayer`，
后者不复制列表、命中即返回——`respawn()` 每 2 秒对所有已加载地图各跑一遍，这条路径上不能有多余分配。
...
| **不加**（地图缺失） | `krex` 541020700、`ulu` 541020500、`ulu2` 541020200 —— `wz/` 与 `wz-zh-CN/` 里都没有，加了只会传送失败。`krex` 与 G7 的克雷塞尔缺图是同一件事。**已于 2026-08-20 补上，见 §27** |
...
### 17.4 `en-US` 不是受支持的配置（用户 2026-08-18 定案）

审查报出「273 个脚本只进了中文层，`en-US` 下对应 NPC/传送点拿到 `null`」。属实，
但成因不是「把功能放错层」，而是**上游没有英文版可放**。

#### 数据

| | 仅存在于 `scripts-zh-CN/` 的脚本 |
|---|---|
| 本分支之前（`befe53346`） | 70 |
| 现在 | 343 |
| **本批引入** | **273** |
...
| 在 ASM 侧**也**只有中文层 | 210 |
| 在 ASM 英文层存在（可以直接取） | **0** |
| 其中属 `BeiDouSpecial/`（北斗自制，本就只该有中文） | 66 |

ASM 自己的分层就是 zh 2272 / en 1921，仅中文层 353 个。零覆盖导入忠实地
复制了这个不对称——**不是导入方式的问题，是源头就没有英文实现**。

#### 机制回顾

`AbstractScriptManager.getInvocableScriptEngine` 按文件级选择：
先找 `scripts-<language>/<path>`，没有才回退 `scripts/<path>`，两者都没有返回 `null`。
所以 `language: en-US` 时 `scripts-zh-CN/` 根本不在查找路径上，
这 343 个脚本对应的 NPC、传送点、反应堆全部无脚本。
...
| 方案 | 代价 |
|---|---|
| **接受，明确不支持 `en-US`**（**选定**） | 零成本。`application.yml:56` 默认 `zh-CN`，生产跑的就是它 |
| 把中文脚本原样拷进 `scripts/` | `en-US` 能跑但对话是中文；且 `scripts/` 从此不再是「英文原版」层，语义塌掉 |
| 翻译 273 个脚本 | 不现实 |

**因此：本项目只支持 `gms.service.language: zh-CN`。** 切 `en-US` 会遇到的不止脚本，
还有 §17.2 记的英文基础层 `wz/Quest.wz` 少 365 条任务、`wz/Etc.wz/Commodity.img` 少 129 条。
要支持 `en-US` 是一个独立项目，不在 LK 移植范围内。

> **写代码时的推论**：从 Java 侧拉起脚本（`openNpc`、`NPCScriptManager.start`）
> 必须接返回值。脚本缺失时返回 `false`/`null`，指令场景提示一句即可，
> 但**玩家点按钮触发的场景（如拍卖行入口）客户端会一直卡在等待**，
> 必须回退到两层都有的脚本。
...
## 19. `Map.wz`/`Mob.wz`/`Npc.wz` 功能性分诊（2026-08-18，§14.3 第 8 项）

§13.2 只说了「互有增删」，没说增删的是什么。本节按**服务端实际会读的节点**
把 349 个差异文件过一遍，给出可执行的取舍。

工具留在 `docs/tools/`（`finddiff.pl` 找差异文件，`triage2.pl` 出分诊），
逐文件账目在 [asm-map-mob-npc-triage.tsv](asm-map-mob-npc-triage.tsv)，
Map 明细在 `docs/tools/asm-triage-detail-map.txt`。

### 19.1 「功能性」的判据

不靠主观分类，取 `MapFactory` / `LifeFactory` 真正解引用的顶层段：
...
差异文件数复算与 §13.1 对得上：`Npc.wz` 16、`Mob.wz` 178、`Map.wz` **155**
（§13.1 记 157，其中 `Map2/211040600` 等 2 张已由 `2797a46f3` 合掉）。

### 19.2 `foothold` 必须排除：是编号差异，不是几何差异

`foothold` 一段占 Map.wz 全部功能性分歧的 **91%**（21,950 / 24,149），且双向对称
（ASM 多 10,592、我们多 8,415）——这个形状本身就说明不是内容差异。

抽 `Map3/300030100` 按**坐标集合**（`x1,y1,x2,y2`）而非编号路径比对：
...
即两棵树的踏板几何基本一致，只是 `foothold/<组>/<id>` 的分组与编号不同。
**按 id 比对 foothold 无意义，合并时整段跳过。**

### 19.3 排除 foothold 后的真实缺口

Map.wz 155 个差异文件里，**67 个**仍有 ASM 独有的功能性内容，合计 **947 条**：

| 段 | ASM 新增 | 我们独有 | 改值 |
|---|---|---|---|
| `life`（刷怪点／NPC 槽） | **506** | 70 | 655 |
| `ladderRope` | 155 | 6 | 81 |
| `info` | 153 | 5 | 167 |
| `portal` | **133** | 56 | 149 |
| `reactor` | 0 | 0 | 12 |

**85% 集中在一个内容块**：`Map3/3000*` 妖精森林 16 张图占 805 条
（我们独有仅 50 条）。剩下 51 张图分摊 142 条。

### 19.4 `Mob.wz`：178 个文件，收益碎
...
ASM 新增的字段以 `elemAttr`(30)、`attack/*`(20)、`ban/*`(18)、`skill/1/hit`(8) 为主，
其余是 `mobType`/`hideHP`/`category`/`PDRate`/`MDRate` 各 5。
**26 个文件我们更全**，不能整树推——只能逐个看。

### 19.5 `Npc.wz`：16 个文件，**一条都不要**

全列出来了，没有一条值得取：

| 类型 | 条数 | 判定 |
|---|---|---|
| ASM 新增 `script` | 3 | `_card_collect`、`圣诞组队任务`、`树妖姥姥` —— **三个脚本文件两边都不存在** |
| `script` 改值 | 4 | **ASM 把脚本名换成了中文描述**，取过来直接打断脚本 |
| `hideName` 0→1 | 8 | 纯显示 |

改值那 4 条的形状：

```
2042000  BD=mc_enter          ASM=怪物嘉年华      ← §7 已定案不做
2042001  BD=mc_enter1         ASM=怪物嘉年华
2042002  BD=mc_move           ASM=怪物嘉年华
9120015  BD=To the Showa...   ASM=前往火狸金融...
9201096  BD=Jack_Additional   ASM=杰克的增益药水
```

`info/script/0/script` 是服务端要拿去 `npc/<name>.js` 找文件的**脚本名**，
不是展示文案。ASM 那侧这几个 NPC 的脚本实际是坏的。**`Npc.wz` 整棵树跳过。**

### 19.6 建议顺序

1. **妖精森林 `Map3/3000*` 16 张**（805 条，占 85%，一个完整内容块，能整体验证）
2. **剩余 Map 中 ASM 新增 ≥ 7 条的 8 张**：`101020001`、`240060000`、`240060100`、
   `222010402`、`261000011`、`261000021`、`240050500`、`240050600`（合计 121 条）
...
LK 的 `handbook/NPC.txt` 里那条给了关键线索：

```
9800001 - MessageBoard (from 9040011)
```

**「from 9040011」说明 LK 自己也没有 9800001 的原始资源，是拿 NPC `9040011` 复制改的。**
落地时可照此办理：从 `Npc.wz/9040011.img.xml` 复制一份改 id，省得去找不存在的原件。
已确认 `gms-server/wz/Npc.wz/9040011.img.xml` **存在**（英文基础层有，中文层无），可直接作为模板。
...
## 27. `@goto` 补齐乌鲁三张图 + 选单改双列（2026-08-20）

用户 2026-08-20 要求「`@goto` 还有哪些 LK 的 map 没迁过来，补一下」，并要求选单每行排两个。

### 27.1 全量比对：LK 相对 BeiDou 只差三条

比对 `GameConstants.GOTO_TOWNS` / `GOTO_AREAS` 两侧的**全部**条目（LK
[`src/constants/game/GameConstants.java:68-144`]，BeiDou 同名文件 `:66-146`）：

| 类别 | 结论 |
|---|---|
| `GOTO_TOWNS` | 逐条对齐，**无缺**。两处键名差异是有意的：LK 的 `korean` = BeiDou 的 `kft`（同为 `222000000` 童话村）；`barber` LK 放在 `GOTO_AREAS`，BeiDou 放在 `GOTO_TOWNS`（美发店对普通玩家开放） |
| `GOTO_AREAS` | 差 **3 条**：`ulu2` `541020200`、`ulu` `541020500`、`krex` `541020700` |
| `temple` | 仍按 G10 的判断保持 `270000100`（入口），不采 LK 的 `270000000`（内层房间） |

差的这三条正是 G10 那张表里「**不加**（地图缺失）」的一行——**当初的理由现在已经不成立**。

### 27.2 原「地图缺失」理由已失效

G10 记的是「`wz/` 与 `wz-zh-CN/` 里都没有，加了只会传送失败」。实测三张图**都在**：

| map id | 文件 | 名字（`wz-zh-CN/String.wz/Map.img.xml`） | 落点 |
|---|---|---|---|
| `541020200` | `wz/Map.wz/Map/Map5/541020200.img.xml` | 乌鲁庄园 II | 有 `pt=0` 出生点 + `life` |
| `541020500` | `wz/Map.wz/Map/Map5/541020500.img.xml` | 乌鲁城中心 | 同上 |
| `541020700` | `wz/Map.wz/Map/Map5/541020700.img.xml` | 克雷塞尔遗迹 I | 同上 |

原因就是 §11 的 **ASM wz 增量导入**——同一批把 `Map5/541020700`、`541020800` 搬进来，
解冻了克雷塞尔整条线（见 §11.5，`event/KrexelBattle.js` 已于 `40712e8db` 落地）。
`krex` 指的 `541020700` 正是 `KrexelBattle.js` 的 `recruitMap`/`exitMap`（远征集合点），
不是 BOSS 房 `541020800`。G10 的结论与 §11.5 的结论之间没人回头对过账，是这次的漏网点。

名字能查到：`MapFactory.getMapStringName` 把 `54102xxxx` 归到 `singapore/<id>`，
三条都在 `wz-zh-CN/String.wz/Map.img.xml` 的 `singapore` 段内。

> **遗留（与 shanghai/shaolin/cjg/wugong 同一条）**：英文基础层 `wz/String.wz/Map.img.xml`
> 没有这三条，en-US 部署下 `@goto` 列表会显示空名。按 §17.4 的定案本项目只支持 zh-CN，
> 补英文名要动 img.xml，留到 wz 批次。

落地：`MapId` 新增 `ULU_ESTATE_2` / `ULU_CITY_CENTER` / `ENTRANCE_TO_KREXEL`
（命名取 `handbook/Map.txt:4463-4465` 的英文条目），`GOTO_AREAS` 三条走常量、仅 GM 可见（与 LK 一致）。

### 27.3 选单重排：双列 + 分段标题 + 定宽对齐

`GOTO_TOWNS` 39 条 + `GOTO_AREAS` 32 条 = **71 个目的地**。一行一个时 GM 看到 71 行，
而**客户端对话框高度固定且没有滚动条，装不下的行直接不画**（同一件事在
`MapleLand/whoDropsList.js` 抬头已记，那边是靠分页解决的），后半张清单等于点不到。

改后（运营 2026-08-20 提的四条）：

| 要求 | 落地 |
|---|---|
| 每行两个 | `PER_LINE = 2`。普通玩家 **20 行**，GM **36 行**（另加 1 行招呼语、2 行标题、1 行空行） |
| 只有英文名上蓝色 | 原来是在开头甩一个 `#b` 把整屏都染蓝。现改成 `#b<英文名>#k <地图名>`，地图名不着色 |
| 城镇 / 区域各带标题并分开 | 段前加 `#r城镇：#k` / `#r区域：#k`，两段之间空一行。区域段原来的 `#r*#k ` 前缀去掉——有标题就不用再逐条打星号了，两段排版彻底一致 |
| 第二列起始位置对齐 | 英文名补到 `KEY_WIDTH = 11`、地图名补到 `NAME_WIDTH = 12`（半角 1、全角 2），超长地图名截断补 `..` |

一格的样子（英文名一栏 11 宽 + 地图名一栏 12 宽 = 23，列间隔 2）：

```
城镇：
henesys    射手村        barber     射手村美发店
ellinia    魔法密林      perion     勇士部落
nautilus   诺特勒斯号..  ereve      圣地
```

#### 为什么补空格这次能对齐

上一版注释里写的是「客户端是比例字体，补了也对不齐」，**这句话对英文客户端成立，对中文客户端不成立**：
中文客户端用的宋体 12 里半角恰好是全角的一半，按「半角 1、全角 2」补空格就能对齐。
本项目只支持 zh-CN（§17.4），按中文客户端的度量排。

行宽预算是**实测来的**，不是猜的：同一种对话框（`sendNextSelectLevel` + `#L..#l`）里，
`scripts-zh-CN/BeiDouSpecial/万能传送.js` 的选项标签最宽 **51 个半角宽**且不折行——
那个脚本本来就是靠空格和全角空格补对齐的，是现成的先例。本表两格并排最宽 **48**，留 3 格余量。

#### 地图名封顶 12 宽（6 个汉字）

71 条里 62 条本来就 ≤ 12 宽，超出的 9 条截断补 `..`：

| 键 | 原名 | 显示 |
|---|---|---|
| `bosspq` | 工作人员强化特别训练场 | 工作人员强.. |
| `guild` | 家族中心(英雄公馆) | 家族中心(.. |
| `horntail` | 暗黑龙王洞穴入口 | 暗黑龙王洞.. |
| `skelegon` | 被遗忘的龙之巢穴 | 被遗忘的龙.. |
| `nlc` | 新叶城-市区中心 | 新叶城-市.. |
| `krex` / `zakum2` / `neo` / `nautilus` | 克雷塞尔遗迹 I / 扎昆的祭台入口 / 塔拉森林时间门 / 诺特勒斯号码头 | 各截到 10 宽 + `..` |

省略号用半角 `..` 而不是 `…`：英文客户端出包字符集是 US-ASCII（同 §G10 `toClientEncodable` 那条），
全角省略号编不出来会变问号。留个记号是必要的——不留玩家会以为地图就叫这名字。

#### 两处实现细节

- **一格长什么样由 Java 说了算**。`GotoCommand.formatCell(key, mapId, padded)` 是唯一的排版口径，
  `PER_LINE` / `COLUMN_GAP` 也从 Java 读；脚本只管分段、排行、套 `#L`。理由与「传送交回
  `warpFromMenu`」一样——`openNpc` 起不来时会退回 `GotoCommand` 的纯文本兜底清单，
  两份排版抄两遍迟早会漂。补白按**地图名**的宽度算，不能拿整格去量：`#b` / `#k` 不占显示宽度。
- **分段排，不拉平**。城镇一段、区域一段各自数行，免得一行左边是城镇右边是区域；
  段末强制换行，否则下一段的标题会接到上一段最后一行的尾巴上。选项序号仍是
  「登记进 `destinations` 时的下标」，`levelGoto` 的回调口径不变。
- **`GotoCommand` 的实例初始化块由 `+=` 改成 `=`**。原来是在实例初始化块里往静态字段追加，
  命令被实例化两次清单就会排两遍（LK 那边是靠 `if (GOTO_TOWNS_INFO == "")` 挡的）。

> **未实测项**：GM 那 40 行（招呼语 + 2 标题 + 空行 + 36 行）是否会顶出对话框高度没有在客户端验过。
> 普通玩家 22 行肯定够。真顶出去的话，把区域段也分页（照 `whoDropsList.js` 的做法）或每行排三格。
