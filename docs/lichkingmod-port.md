...
**GM 不算人**（2026-08-21 追加，入口 `MapleMap#getSpawnRelevantPlayers`）：
只要图里有普通玩家，`gmLevel > 1` 的角色就一律排除在刷怪计算之外，等同于不在本图——
既不计入「有效玩家数」，也不会用自己的等级顶高 maxLevel 把在场的低等级玩家挤出有效玩家范围。

**例外**：图里一个普通玩家都没有时，GM 按普通人计。否则 GM 独自进图会让刷怪率算作 0 人、
整张图停止补怪，巡图和实测都做不了。多个 GM 同处一图按实际人数计。
因为有这条例外，`respawn()` 的前置判断只需判空图（有人就有人算），不必再单独识别 GM。
